...
public:
    ...
    std::string Handle(const ygsoul::wifi_scan::Request& request, bool may_start) {
        ...
        std::lock_guard<std::mutex> lock(mutex_);
        if (!ValidId(request.id) || !ValidId(request.scan) ||
            (request.op != "start" && request.op != "get" && request.op != "cancel") ||
            request.index < -1 || request.index >= static_cast<int>(kMaxNetworks))
            return ErrorReply(request.id, request.scan, Error::InvalidRequest);
        if (!accepting_) return ErrorReply(request.id, request.scan, Error::Busy);
        if (request.op == "start") {
            if (!may_start) return ErrorReply(request.id, request.scan, Error::Busy);
            // Retrying an acknowledged start does not rescan or change the snapshot.
            if (request.scan == scan_id_ && state_ != State::Idle)
                return Reply(request.id, scan_id_, state_, rows_, -1, truncated_, error_);
            if (running_) return ErrorReply(request.id, request.scan, Error::Busy);
            ResetLocked();
            scan_id_ = request.scan;
            ...
            if (request.scan != scan_id_ || state_ == State::Idle)
                return ErrorReply(request.id, request.scan, Error::StaleScan);
        ...
        return Reply(request.id, request.scan, state_, rows_, request.op == "get" ? request.index : -1,
                     truncated_, error_);
    }
...
private:
    ...
    void Run() {
        ...
        if (failure == Error::None && Current(generation)) {
            if ((!manager.IsInitialized() && !manager.Initialize()) || manager.IsConfigMode()) {
                ...
                if (Current(generation)) {
                    ...
                    if (driver_error != ESP_OK) failure = Error::Driver;
                }
            }
        }
        ...
        for (int channel = country.schan; failure == Error::None && Current(generation) && channel <= last_channel; ++channel) {
            ...
            if (driver_error != ESP_OK) { failure = Error::Driver; break; }
            uint16_t available = 0;
            driver_error = esp_wifi_scan_get_ap_num(&available);
            if (driver_error != ESP_OK) {
                esp_wifi_clear_ap_list(); failure = Error::Driver; break;
            }
            ...
            for (uint16_t index = 0; index < count; ++index) {
                ...
                AddAccessPoint(rows, {
                    std::string(reinterpret_cast<const char*>(ap.ssid), strnlen(reinterpret_cast<const char*>(ap.ssid), 32)),
                    Hex(std::string(reinterpret_cast<const char*>(ap.bssid), 6)),
                    ap.rssi, ap.primary, static_cast<int>(ap.authmode)
                }, truncated);
            }
        }
        ...
        if (own_radio) {
            ...
            if (stop_error != ESP_OK && failure == Error::None) { failure = Error::Driver; driver_error = stop_error; }
        }
        if (failure != Error::None) ESP_LOGW("YGWifiScan", "scan failed: %s (%d)", ErrorName(failure), static_cast<int>(driver_error));
        {
            ...
            if (accepting_ && generation_ == generation) {
                error_ = failure;
                state_ = failure == Error::None ? State::Done : State::Failed;
